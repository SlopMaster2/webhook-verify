...
- The `test-nostd` `http` CI run (`cargo test --no-default-features
  ...
  (issue #25) now ships: the `no-std-wasm` CI job build-checks both
  `sendgrid,paypal` and `http` feature sets for `--target
  wasm32-unknown-unknown` (parity with the `test-nostd` matrix).
