...
//!
//! Scheme, per Calendly's official documentation
...
//!   (the docs' reference implementations build `t + '.' + request.body` and
//!   warn that parsing the JSON payload before verification breaks the
//!   signature).
//! - Algorithm: HMAC-SHA256 keyed by the subscription's webhook signing key as
//!   a plain UTF-8 string, hex-encoded, carried bare in the header (no
//!   `sha256=` prefix).
...
//! # Replay protection
//!
//! Calendly's docs demonstrate a 180-second tolerance (`three_minutes = 180`);
//! the crate's shared window applies here — `|now - t|` against
//! [`VerifyOptions::max_age`] (default 300s) using the injected clock. Callers
//! wanting the documented 180-second zone set the option explicitly. The
//! symmetric check also rejects future-dated timestamps, which no legitimate
//! delivery produces; the docs' examples only reject the old half, but the
//! crate's default tolerance applies as with Slack, Zoom, Cloudflare, Coinbase,
//! and Mux.
//!
//! Note: the `t` value rides *inside* the single `Calendly-Webhook-Signature`
...
/// The header carrying Calendly's combined `t` and `v1` fields.
pub(crate) const SIGNATURE_HEADER: &str = "Calendly-Webhook-Signature";

/// The `t` field name inside [`SIGNATURE_HEADER`]'s comma-separated list.
const TIME_FIELD: &str = "t";

/// The only signature scheme Calendly defines, per its docs.
const SCHEME: &str = "v1";
...
/// A successfully parsed `Calendly-Webhook-Signature` header value.
...
/// Parses the header per Calendly's documented algorithm: split on `,`, split
...
/// closed on ambiguity. Calendly's docs define exactly one signature element
/// and no rotation window, so a second `v1=` is treated as malformed rather
/// than rotation.
...
mod tests {
    ...
    /// A signing key used verbatim as its UTF-8 bytes (Calendly never decodes
    /// it).
    const SECRET: &str = "calendly_signing_key_test";

    const BODY: &[u8] =
        b"{\"event\":\"invitee.created\",\"payload\":{\"email\":\"test@example.com\"}}";

    const TIME: u64 = 1_700_000_000;

    /// Locally constructed over the documented `{t}.{raw_body}` construction:
    /// `printf '1700000000.{"event":"invitee.created","payload":{"email":"test@example.com"}}' \
    ///   | openssl dgst -sha256 -hmac "calendly_signing_key_test"`.
    ///
    /// Cross-checked with Python's `hmac.new(b"calendly_signing_key_test",
    /// b"1700000000." + body, hashlib.sha256)`. Calendly's docs publish no
    /// byte-exact example signature over a body, but they do publish the exact
    /// signed-string construction and header layout, so the vectors follow
    /// them precisely.
    const SIGNATURE: &str = "c28fe5f7a1527d7f93349786637c707d636115f902962c743a8b8d88b6b9c401";

    /// Locally constructed over an empty body (boundary case).
    const EMPTY_BODY_SIGNATURE: &str =
        "eb06208e5d2a3140b5929eed06d0f66314266864d4d4ee6bf26cdb3855f21f58";

    /// Locally constructed over `"éé🦀"` (unicode boundary case).
    const UNICODE_BODY_SIGNATURE: &str =
        "24d034122de009b3d01986e5c6277d6fa6996f7de32698be7d06db86c429a3fe";

    /// The full example header published in Calendly's docs
    /// (<https://developer.calendly.com/api-docs/overview/webhooks/webhook-signatures>:
    /// `Calendly-Webhook-Signature: t=1492774577,v1=5257a869...b8bd`).
    /// Calendly publishes no body or signing key for it, so it is replayed as a
    ...
    /// verifies, and the docs' published example parses but mismatches.
    #[test]
    ...
    fn docs_example_header_is_well_formed_but_mismatches() {
        // Calendly's published example header must parse as a perfectly valid
        ...
        let result = verify_with(
            ...
            DOCS_EXAMPLE_HEADER,
            &Secret::new(SECRET),
            clocked_at(1_492_774_577, Some(Duration::from_secs(180))),
        );
    ...
    fn tampered_body_fails() {
        ...
        assert_eq!(
            verify_fresh(b"{\"event\":\"invitee.canceled\"}", SIGNATURE),
            Err(VerifyError::SignatureMismatch)
        );
    }
    ...
    fn wrong_secret_fails() {
        let result = verify_with(
            ...
            &Secret::new("not-the-signing-key"),
            clocked_at(TIME, Some(Duration::from_secs(300))),
        );
        assert_eq!(result, Err(VerifyError::SignatureMismatch));
    }
    ...
    #[test]
    fn replay_documented_180s_tolerance_is_configurable() {
        // Calendly's docs use a 180-second zone; with that configured, a
        // 181s-old delivery is rejected while a 180s-old one is accepted.
    ...
    fn malformed_signature_header_errors_distinctly() {
        let cases: Vec<(String, VerifyError)> = vec![
            ...
            // Ambiguous duplicate `v1` field: Calendly documents no rotation
